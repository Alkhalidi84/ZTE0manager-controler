 ...
 *   - authenticated POST actions (fetch RD, compute AD, attach AD),
...
export class GoformClient {
  ...
  /**
   * Fire an action command, attaching the AD token unless disabled.
   *
  ...
  private async sendSet(
    ...
    if (authenticated) {
      try {
        // ZTE's stock web UI uses RD only as the nonce/input for AD.
        // It sends AD to the router, but does NOT include RD in the POST body.
        // The G5B firmware rejects BAND_SELECT when RD is also posted.
        const { ad } = await this.buildTokens();
        body.set('AD', ad);
      } catch (err) {
