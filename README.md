...
- **API Explorer / Console / Developer Mode** — auto‑discovers the router's hidden commands.

### Get the app

- **🪟 Windows** — prebuilt `.exe` in `release/…`, or build it: `npm install && npm run dist:win`.
...
4. Install it on your phone (enable *Install unknown apps*), and connect the phone to the **router's Wi‑Fi**.
...
Node 18+ to build. Java 17 + Android SDK are needed only if you build the APK locally.
...
- **🪟 ويندوز** — ملف `.exe` جاهز في مجلد `release/…`، أو ابنِه: `npm install && npm run dist:win`.
- **🤖 أندرويد** — ابنِ ملف APK مجاني بالسحابة (الخطوات تحت، بدون تثبيت أدوات).
...
Local‑only. No cloud. No telemetry. Your credentials and data stay on your device
and only reach your own router.

محلي فقط، بدون سحابة ولا تتبّع، وبياناتك تبقى على جهازك وتصل راوترك فقط.

---

MIT License · For use on routers you own or are authorized to manage.
